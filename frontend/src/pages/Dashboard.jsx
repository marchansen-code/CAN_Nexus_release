...
// Expired Article Row - simple one-line display with dismiss button
const ExpiredArticleRow = ({ article, onClick, onDismiss }) => {
  const formatDate = (dateString) => {
    if (!dateString) return "";
    const date = new Date(dateString);
    return date.toLocaleDateString("de-DE", {
  ...
  const daysExpired = article.days_expired || 0;
  
  return (
    <div
      className="flex items-center justify-between p-2 hover:bg-red-100/50 dark:hover:bg-red-900/30 transition-colors border-b last:border-b-0"
      data-testid={`expired-article-${article.article_id}`}
    >
      <div 
      ...
      >
        ...
        <Badge variant="outline" className="text-[10px] px-1 py-0 shrink-0 border-red-300 text-red-600">
          {daysExpired === 0 ? "Heute abgelaufen" : daysExpired === 1 ? "Seit 1 Tag" : `Seit ${daysExpired} Tagen`}
        </Badge>
      </div>
      <Button
        size="sm"
        ...
        onClick={(e) => {
          e.stopPropagation();
          onDismiss(article.article_id);
        }}
        title="Ausblenden"
        data-testid={`dismiss-expired-${article.article_id}`}
      >
...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setExpiredArticles(statsRes.data.expired_articles || []);
  ...
  // Dismiss expired article notification
  const handleDismissExpired = async (articleId) => {
    try {
      await axios.post(`${API}/stats/dismiss-expired`, { article_ids: [articleId] });
      setExpiredArticles(prev => prev.filter(a => a.article_id !== articleId));
      toast.success("Hinweis ausgeblendet");
    } catch (error) {
      console.error("Failed to dismiss expired article:", error);
      toast.error("Fehler beim Ausblenden");
    }
  };

  // Dismiss all expired articles
  const handleDismissAllExpired = async () => {
    try {
      const articleIds = expiredArticles.map(a => a.article_id);
      await axios.post(`${API}/stats/dismiss-expired`, { article_ids: articleIds });
      setExpiredArticles([]);
      toast.success("Alle Hinweise ausgeblendet");
    } catch (error) {
      console.error("Failed to dismiss all expired articles:", error);
      toast.error("Fehler beim Ausblenden");
    }
  };
  ...
  return (
    <div className="space-y-6 animate-fadeIn" data-testid="dashboard">
      ...
      {/* Pinnwand Section - Always at the top */}
      {pinnwandCategories.length > 0 && (
        <Card className="border-t-4 border-t-amber-500" data-testid="pinnwand-section">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {Object.values(articlesByCategory).map(({ category, title, articles }) => {
                ...
                return (
                  <Card key={category.category_id} className="border-l-4 border-l-amber-400 h-fit shadow-sm">
      ...
      {/* Expiring and Expired Articles Row */}
      {(stats?.expiring_articles?.length > 0 || expiredArticles.length > 0) && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {expiredArticles.length > 0 && (
            <Card className="border-l-4 border-l-red-500 bg-gradient-to-r from-red-50 to-transparent dark:from-red-900/20 dark:to-transparent" data-testid="expired-articles-section">
              <CardHeader className="flex flex-row items-center justify-between pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  <XCircle className="w-5 h-5 text-red-500" />
                  <span>Abgelaufen</span>
                  <Badge variant="secondary" className="ml-2 bg-red-100 text-red-700 dark:bg-red-800 dark:text-red-200">
                ...
                <Button
                  ...
                  onClick={handleDismissAllExpired}
                  data-testid="dismiss-all-expired"
                >
              ...
              <CardContent className="pt-0">
                <div className="rounded-lg border border-red-200 dark:border-red-800 max-h-64 overflow-y-auto">
